...
import ConfirmDialog from "./components/task-app/ConfirmDialog.jsx";
...
export default function TaskApp({ onLogout }) {
  ...
  // The user-editable project list lives on settings. Only fall back to the
  // static seed list while settings haven't loaded yet — once the backend
  // responds, even an empty array is a valid explicit choice and we respect it.
  const projectOptions = useMemo(() => {
    const fromSettings = settings.projects;
    return Array.isArray(fromSettings) ? fromSettings : CATEGORY_OPTIONS;
  }, [settings.projects]);
  ...
  async function handleNewTask(event) {
    ...
    try {
      ...
      setTaskSubmitError("");
      await addItem({ text, category: composer.category, due: composer.due.trim() || "", flagged: composer.flagged, bucket: composer.bucket });
      ...
      setTaskSubmitError(submitError.message || "Could not create the task.");
    } finally {
  ...
  function handleRequestDeleteTask(id) {
  ...
  return (
    <main className="h-screen overflow-hidden bg-[#eef3f7] text-slate-900">
            ...
            <div className="grid h-full gap-4 px-5 py-4 xl:grid-cols-[minmax(0,1fr)_220px]">
              <section className="flex min-h-0 flex-col">
                ...
                <TaskTable
                  ...
                  onDeleteItem={handleRequestDeleteTask}
                  onEditItem={setEditingItem}
                  emptyTitle={loading ? "Loading tasks" : activeSection.emptyTitle}
                  emptyDescription={loading ? "Fetching your workspace from the backend." : activeSection.emptyDescription}
                />
              </section>
      ...
      <ConfirmDialog
        ...
        message={
          taskToDelete
            ? `"${taskToDelete.text}" will be removed. This can't be undone.`
            : ""
        }
